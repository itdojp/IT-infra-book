...
各層は特定の責任を持つ。
- **物理層**：ビットの伝送
- **データリンク層**：隣接ノード間のフレーム転送（媒体アクセス制御、誤り検出など）
- **ネットワーク層**：エンドツーエンドの経路制御
- **トランスポート層**：プロセス間の通信（順序制御、再送、輻輳制御など。TCPの場合）
...
![図1-1: OSI参照モデルの7層とTCP/IPモデルの4層の対応関係]({{ '/assets/images/diagrams/chapter01/osi-tcpip-comparison.svg' | relative_url }})\
*図1-1: OSI参照モデルとTCP/IPモデルの比較*
{: #figure-1-1 .book-figure}
...
![図1-2: 送信側で各層のヘッダーを付加し、受信側で逆順に除去するプロセス]({{ '/assets/images/diagrams/chapter01/encapsulation-decapsulation.svg' | relative_url }})\
*図1-2: カプセル化とデカプセル化のプロセス*
{: #figure-1-2 .book-figure}
...
ゼロコピーは、データ転送時のメモリコピー回数を減らす最適化であり、必ずしもカーネルを迂回することを意味しない。
```text
利点：
- メモリコピーの削減（例: 2回 → 1回。sendfile によるファイル→ソケット転送や MSG_ZEROCOPY を用いた大容量送信などの条件下では 0 回まで削減可能）
- CPUキャッシュの有効活用
- レイテンシの改善
...
- sendfile/splice
- Linux MSG_ZEROCOPY
- io_uring（登録バッファ使用時など、特定の条件でコピー回避に寄与）
```

**カーネルバイパス（特定条件下）**

カーネルネットワークスタックを迂回してユーザー空間でパケット処理を行う方式である（要件・制約が大きい）。
```text
実装例：
...
![図1-3: ネットワークスタックのCPU、メモリ、I/O処理の相互作用と性能への影響]({{ '/assets/images/diagrams/chapter01/layer-performance-characteristics.svg' | relative_url }})\
*図1-3: レイヤー間相互作用とパフォーマンス特性*
{: #figure-1-3 .book-figure}
...
```text
層間協調による効率化（層分離は維持）：
- チェックサム計算の最適化：TCP/IPチェックサムの並列計算
- プロトコル処理の効率化：同一CPUコアでの連続処理
- バッファ管理の統合：レイヤー間でのゼロコピーバッファ共有
- ルーティングキャッシュ：IP層のルート情報をTCP層で活用

効果：処理時間を20〜30%削減（層の責任分離は維持）
```
...
```text
脅威：
- 盗聴（光ファイバータッピング）：光ファイバーケーブルに不正に接続して通信内容を傍受される
- 電磁波漏洩：通信ケーブルから漏れた電磁波を受信され、内容を推測される
- ケーブルの物理的切断：ケーブル破壊により通信停止や障害が発生する

対策：
- 暗号化された光伝送：盗聴されても内容が読めないように暗号化する
- シールドケーブルの使用：電磁波漏洩を抑制する
- 物理的アクセス制御：サーバールームや配線経路への立ち入りを制限する
```
