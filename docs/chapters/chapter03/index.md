...
本章では、L3ネットワーク（IP）の設計原理として、アドレス設計・ルーティング・NATの各要素がどのように関連し、どこに制約やトレードオフがあるのかを整理する。

本章を読み終えた時点で、読者は以下のことができるようになる。
- IPv4/IPv6移行やCIDR/VLSMを踏まえ、アドレス割り当てとサブネット設計の方針を立てられる
- スタティックルーティングとダイナミックルーティングの適用範囲を判断できる
- ルーティングプロトコル選定の評価軸（収束、運用、スケール等）を説明できる
- NAT/NAPTの利点と副作用を整理し、設計上の影響を見積もれる

## 3.1 IP アドレス体系の設計制約

### クラスフルからCIDRへの必然的進化

IP アドレスの初期設計であるクラスフルアドレッシングは、1980年代のネットワーク規模を前提としていた。しかし、インターネットの急速な拡大により、この設計の限界が明らかになった。CIDR（Classless Inter-Domain Routing）への移行は、単なる技術的改良ではなく、アドレス空間の枯渇という実存的危機への対応だった。
...
**IPv4枯渇のタイムライン（基準日: 2026年2月時点。配布状況やポリシーは変動するため、最新は一次情報を参照）**
...
参考（一次情報）：
- [RIPE NCC IPv4 run-out](https://www.ripe.net/manage-ips-and-asns/ipv4/ipv4-run-out/)
- [JPNIC IPv4アドレス枯渇関連](https://www.nic.ad.jp/en/ip/ipv4pool/)
...
```text
例：部門別アドレス設計
技術部（120台）：192.168.1.0/25   （126台分）
営業部（50台）： 192.168.1.128/26 （62台分）
管理部（20台）： 192.168.1.192/27 （30台分）
```

最小実験（サブネット境界の検算例）：
```bash
# ipcalc は環境により未導入の場合がある
ipcalc 192.168.1.0/25
ipcalc 192.168.1.128/26
ipcalc 192.168.1.192/27
# 各サブネットのNetworkとBroadcastが互いに重複していないことを確認する
```
...
スタティックルートは、宛先ネットワークと次ホップ（または出力インターフェース）を、管理者が手動で設定する方式である。

```bash
# 例: 10.0.0.0/24 への経路を 192.168.1.1 に向ける（Linuxの例）
