...
IP アドレスという数字の羅列を、人間が理解できる名前に変換するDNS。この一見単純な機能の背後には、インターネット規模での分散データベースという壮大な仕組みが存在している。なぜDNSは階層的な構造を採用したのか、なぜ単一の巨大なデータベースではいけなかったのか。
...
- 動的なホスト管理を支える仕組みと、設計・実装の要点を理解できる
- DNS以外のサービス発見手段も含め、選択基準を整理できる
...
3. **プリフェッチング**
   ```text
   ※プリフェッチや期限切れ後の応答継続は、再帰リゾルバの「実装/設定」に依存する（標準DNSの必須挙動ではない）

   実装例（再帰リゾルバの機能）：
   - prefetch: TTL 期限切れ前にバックグラウンド更新し、レイテンシを平準化する
   - serve-stale / serve-expired: 権威側が一時的に不調な場合に、期限切れキャッシュを条件付きで返す（RFC 8767）
...
```text
正常なDNS 解決：
1. クライアント → ローカルDNS：www.example.com のIP アドレス問い合わせ
2. ローカルDNS → 権威DNS：www.example.com のIP アドレス問い合わせ
...
DNSビューは、クライアントのIP アドレスや特定の条件に基づいて、異なるDNS応答を提供する機能である。細粒度のアクセス制御と情報隠蔽を実現する。
...
DHCP（Dynamic Host Configuration Protocol）は、ネットワーク設定の自動配布を実現する。適切なスコープ設計とフェイルオーバー機能により、大規模環境での安定したIP アドレス管理を実現する。
