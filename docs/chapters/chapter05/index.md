...
- TLS実装における運用上の論点（終端方式、証明書、性能影響等）を把握できる

**注意（重要）**：本章の `sysctl` / `/proc/sys` の変更例は、システム全体の通信特性に影響する。検証環境でのみ実施し、変更前の値を控えてロールバック可能にすること。永続化する場合は `/etc/sysctl.d/` 等で管理し、運用ポリシーに合わせて適用すること（要確認）。
...
**1. IP アドレスベース親和性（Source IP Affinity）**
...
- [第6章 オペレーティングシステムの内部構造](../chapter06/)
- [目次に戻る](../../)
