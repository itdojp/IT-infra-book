...
- 代表的なカーネルチューニングの判断基準とリスクを理解できる

**注意（重要）**：本章の例には、`sysctl` / `/proc` / `/sys` / cgroup 設定など、ホストの挙動に影響する操作が含まれる。検証環境でのみ実施し、実施前後の値を記録してロールバック可能にすること。特に本番環境への適用は、SLO/セキュリティ要件と整合するかを確認したうえで段階的に行うこと（要確認）。
...
セキュアブート（Secure Boot）は UEFI の機能で、EFI 実行ファイル（例：shim やブートローダー）の署名を検証し、起動チェーン上の各段階で署名検証を行うことで「信頼の連鎖（chain of trust）」を確立する仕組みである。Linux では、署名済み shim を起点に、GRUB や署名付きカーネルを起動する構成（MOK による追加キー登録を含む）が代表例である。
