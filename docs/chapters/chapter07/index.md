...
ストレージデバイスの物理特性（例: 物理セクターサイズ）に合わせたアライメント設定は、性能に大きな影響を与える。近年は物理セクターが4KBのストレージが多く、互換性のため論理セクターを512Bとして見せる（512e）場合もある。アライメントがずれると、1回の論理書き込みが複数の物理書き込み（Read-Modify-Write）に分割され、書き込みレイテンシや書き込み増幅が増える。
