...
本章では、ストレージを「性能」「信頼性」「運用」の観点から捉え、ブロック層からバックアップまでの設計判断を行えるように整理する。

本章を読み終えた時点で、読者は以下のことができるようになる：
- ブロックデバイスの抽象化層（LVM等）の役割と、利用時の注意点を説明できる
- RAIDレベルの特性を比較し、適用条件を整理できる
- ファイルシステムの設計思想の違いを踏まえ、選択基準を説明できる
- ストレージ性能の評価観点（IOPS/スループット/レイテンシ等）を整理できる
- RPO/RTOを前提に、バックアップアーキテクチャの方針を立てられる

**注意（重要）**：本章の `pvcreate` / `vgcreate` / `lvcreate` / `mdadm` / `mkfs` / `dd` / `fio` などは、対象デバイスのデータを消去・破壊し得る。必ず検証環境で実施し、`lsblk` 等でデバイス名を確認したうえで実行すること。
...
# 特性：
# 複数ディスクにデータを分散して保存し、読み書きを並列化して性能を向上させる
# 容量効率：100%（全ディスク容量を利用）
# 耐障害性：なし（1台故障で全データ喪失）
# 性能：読み書き共に台数倍
...
```bash
# 特性：
# 同じデータを複数ディスクへ複製（ミラー）し、冗長性を確保する
# 容量効率：50%（半分の容量がミラー用）
# 耐障害性：1台故障まで許容
# 読み込み性能：約2倍（並列読み込み）
# 書き込み性能：単一ディスクと同等
...
ストレージデバイスの物理特性（例: 物理セクターサイズ）に合わせたアライメント設定は、性能に大きな影響を与える。近年は物理セクターが4KBのストレージが多い。物理/論理とも4KBの 4Kn、または互換性のため論理セクターを512Bとして見せる 512e（物理4KB/論理512B）が代表的である。アライメントがずれると、1回の論理書き込みが複数の物理書き込み（Read-Modify-Write）に分割され、書き込みレイテンシや書き込み増幅が増える。
