...
- 要件に応じてVM/コンテナの選択基準を説明できる

**注意（重要）**：本章のコマンド例には、ホスト設定の変更（Huge Pages、KSM、CPUピニング、SR-IOV、PCIデバイスのunbind 等）を含む。誤るとネットワーク断や起動不能を招き得るため、検証環境でのみ実施し、実施前後の値の記録とロールバック手順を用意すること。

## 8.0 VM/コンテナ選定の判断基準（概要）

仮想化方式の選定は「隔離」「互換性」「性能」「運用」のトレードオフで決まる。以下は最小の判断基準である。

- 隔離: テナント境界が厳格（規制/機密）な場合は VM を優先する。コンテナはカーネル共有のため、境界が弱い。
- 互換性: 異なるOSや、カーネル機能への依存（カーネルモジュール等）がある場合は VM を選ぶ。
- 起動/密度: 高密度や高速起動が重要な場合はコンテナを優先する。
- 運用: パッチ適用の責任範囲（ゲストOS更新の要否）、観測性、バックアップ/復旧手段を先に決める。

失敗例（例）:
- 隔離要件が高いワークロードをコンテナで混在させ、権限境界の例外が増える
- host側のHuge Pages/SR-IOV設定を本番で試し、リモート接続が切れて復旧できない

代替案（例）:
- VMほど重くしたくないが隔離を強めたい場合、microVM 等の「軽量VM」方式を検討する
- 物理分離が必要な場合、ノードプール分離やクラスタ分離で境界を強化する
