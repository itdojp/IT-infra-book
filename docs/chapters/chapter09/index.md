...
コンピュータがコンピュータを管理する。  
この一見単純な概念が、現代のシステム運用の根幹を支えている。

人間による手動運用から自動化への移行は、単なる効率化のためだけではない。システムの規模と複雑性が人間の認知限界を超えつつあるなかで、安定した運用を続けるために必然的に求められた進化だと考えられる。

なぜ init システムから systemd への移行が業界に大きな議論を巻き起こしたのか。ログ管理において、テキストファイルからバイナリ形式への変更がなぜ重要なのか。これらの変化の背景には、運用規模の拡大と、それに伴う運用品質への要求の高度化がある。

## 本章の目的と到達点

本章では、運用自動化の対象を「サービス管理」「ログ/メトリクス」「設定管理」「オーケストレーション」「障害対応」に分解し、設計と運用の論点を整理する。
...
- 障害対応の自動化（初動・診断・復旧）の設計上の論点を理解できる
...
journald は、これらの課題を解決するために設計されたコンポーネントである。従来の syslog ベースの仕組みと比べて、次のような点で改善が期待できる：
...
Prometheus は、サーバやアプリケーションの状態（CPU 使用率、リクエスト数などのメトリクス）を収集・保存するオープンソースの監視システムである。現場では事実上の標準（de facto standard）として広く使われている。Prometheus の設計思想と実装を理解することで、効果的な監視システムを構築できる。
...
Kubernetes は、コンテナ化されたアプリケーションを複数ノード上でデプロイ、スケーリング、自己修復するオープンソースのコンテナオーケストレーションプラットフォームである。現場での標準的な選択肢となっている一方、その設計には多くの考慮点がある。適切な設計なしには、複雑性がメリットを上回る結果となる。
