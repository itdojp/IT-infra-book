...
本章では、セキュリティをアーキテクチャ要素として捉え、分離設計・境界制御・認証/認可・暗号化・監視までを実装観点で整理する。

本章を読み終えた時点で、読者は以下のことができるようになる：
- セグメンテーションの狙いと実装アプローチを説明できる
- ファイアウォール実装における設計観点（ルール設計、性能、運用）を整理できる
- 認証・認可システムの基本構造と、実装パターンを比較できる
- 暗号化実装の論点（鍵管理、終端方式、性能影響等）を把握できる
- セキュリティ監視の要素（ログ、検知、アラート）を整理できる
...
# 高精度検知ルールの例
#
# NOTE: ルールファイルは「1ルール=1行」が基本です。以下は可読性のため折り返して表示しています。
...
alert tcp $HOME_NET any -> $EXTERNAL_NET ![80,443,22,25,53] (
  msg:"Unusual Outbound Connection";
  flags:S;
