...
技術選択は、候補製品やOSSの機能比較表だけで完了しない。インフラ全体設計では、選択した技術が
ネットワーク、OS、ストレージ、セキュリティ、運用、自動化、高可用性の各章で説明した前提と
矛盾しないことを確認する必要がある。採用判断の前に、少なくとも以下をレビュー証跡として残す。

- 適合条件: 解決したい課題、必須要件、採用しない条件、代替案、撤退条件を明文化している。
- 性能・容量: ベンチマーク条件、ピーク負荷、スケール方針、容量増設時の手順と停止影響を説明できる。
- セキュリティ: 信頼境界、権限モデル、秘密情報の保管、ログの機微情報、監査証跡、脆弱性対応責任を確認している。
- 可用性・DR: 冗長化方式、単一障害点、バックアップ、リストア演習、RPO/RTO、フェイルバック条件を記録している。
- 運用性: 監視指標、アラート基準、Runbook、変更管理、障害切り分け、アップグレード手順を用意している。
- 移行・ロールバック: データ移行、互換性、段階リリース、切り戻し、既存環境との並行運用期間を定義している。
- 継続性: ベンダーロックイン、ライセンス、サポート期限、OSSの保守状況、担当者のスキル育成計画を評価している。

レビューでは、Linux基盤、インフラソフトウェア、クラウド、コンテナ、Kubernetesなど後続・関連書籍の
責務分界も合わせて確認する。PRでは未解決レビューコメントを0件にし、CI、ビルド、リンク確認の結果を
採用判断の証跡に含める。
